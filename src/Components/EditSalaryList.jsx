...
// --- ISOLATED SEARCH COMPONENT (Same as Add) ---
...
const EditSalaryList = () => {
  ...
  // --- 1. FETCH & PRE-FILL DATA ---
  useEffect(() => {
    const fetchData = async () => {
      try {
        setIsFetchingData(true);
        
        // 1. Fetch Dropdown Data & The Specific Salary Record in Parallel
        const [projectsRes, employeesRes, salaryRes] = await Promise.all([
          axios.get("/api/project/active-projects"),
          axios.get("/api/attendance/employee"),
          axios.get(`/api/salary/salary-list/${id}/edit`) // Ensure this API endpoint exists
        ]);

        const projectData = projectsRes.data.data || [];
        const employeeData = employeesRes.data.employees || [];
        const existingData = salaryRes.data.data; // Adjust based on your API response structure
        ...
        // 2. Pre-fill Header
        setHeaderData({
          date: existingData.date ? new Date(existingData.date).toISOString().slice(0, 7) : "",
          projectId: existingData.projectId?._id || existingData.projectId, // Handle populated or raw ID
          projectName: existingData.projectName,
          foremanId: existingData.foremanId?._id || existingData.foremanId,
          foremanName: existingData.foremanName
        });

        // 3. Pre-fill Rows
        const formattedRows = existingData.employeeList.map((item) => ({
          tempId: item._id || Date.now() + Math.random(), // Use existing ID or generate one
        ...
        errorToast("Failed to load salary record. It may have been deleted.");
        // router.back(); // Optional: Redirect back on error
      } finally {
  ...
  const handleClearRow = (index) => {
    setRows(prevRows => {
      const newRows = [...prevRows];
      newRows[index] = { ...newRows[index], name: "", employeeId: "", iqama: "", salary: "", isSearchOpen: false };
      return newRows;
    });
  };
  ...
  const handleSelectEmployee = (index, employee) => {
    ...
    setRows(prevRows => {
      ...
      newRows[index] = {
        ...
        status: "pending", // Reset status on change? Or keep existing? Usually reset.
        isSearchOpen: false 
      };
      return newRows;
    });
  ...
  const addRow = () => {
    setRows(prev => [
      ...prev.map(r => ({...r, isSearchOpen: false})),
  ...
  // --- SUBMIT HANDLER (UPDATED FOR EDIT) ---
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // CHANGED: Using PUT and the Edit API endpoint
      const response = await axios.put(`/api/salary/salary-list/edit/${id}`, payload);
      
      if (!response.data.success) {
        errorToast(response.data.message || "Failed to update");
      } else {
        sucessToast("Salary list updated successfully!");
        router.back();
      }
    } catch (error) {
      errorToast(error.response?.data?.message || "Failed to update salary list");
    } finally {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-base-100">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm font-medium text-base-content/60">Loading salary record...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-base-200/50 font-sans text-base-content">
      <div className=" mx-auto md:p-5 p-4 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-2xl md:text-3xl font-bold tracking-tight text-base-content">Edit Salary Sheet</h2>
            <p className="text-sm md:text-base text-base-content/60 mt-1">Update existing salary records.</p>
          </div>
        ...
        <div className=" overflow-visible">
                ...
                <div key={row.tempId} className="bg-base-100 rounded-lg border border-base-200 shadow-sm p-4 relative overflow-visible">
                    ...
                    <div className="space-y-3">
                        <div className="form-control">
                            ...
                            <EmployeeSearchInput 
                              row={row} index={index} employees={employees} 
                              onSearchChange={handleSearchChange} onSelect={handleSelectEmployee} onClear={handleClearRow}
                            />
                        </div>
          ...
          {/* FOOTER */}
          <div className="p-3 md:p-3  border-t border-base-300 flex justify-end gap-3 sticky bottom-0 z-10 md:relative">
            ...
            <button type="button" onClick={handleSubmit} disabled={isSubmitting} className="btn bg-[var(--primary-color)] text-white hover:opacity-90 border-none w-full md:w-auto">
               {isSubmitting ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Update Sheet'}
            </button>
          </div>
